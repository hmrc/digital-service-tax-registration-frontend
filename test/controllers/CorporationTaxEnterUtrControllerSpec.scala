...
class CorporationTaxEnterUtrControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val corporationTaxEnterUtrRoute: String = routes.CorporationTaxEnterUtrController.onPageLoad(NormalMode).url
  ...
  "CorporationTaxEnterUtr Controller" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      when(mockDstService.lookupCompany(any[String], any[String])(using any[HeaderCarrier], any[ExecutionContext]))
    ...
    "must save Company Details from Lookup when valid data is submitted" in {
      ...
      when(mockSessionRepository.set(any())).thenReturn(Future.successful(true))
      when(
        mockDstService.lookupCompany(any[String], any[String])(using any[HeaderCarrier], any[ExecutionContext])
      ...
      when(mockUserAnswers.set(any(), any())(using any())).thenReturn(Try(mockUserAnswers))
      ...
      running(application) {
        ...
        verify(mockUserAnswers).set(ArgumentMatchers.eq(CompanyNamePage), ArgumentMatchers.eq(companyName))(using any())
        verify(mockUserAnswers)
          .set(ArgumentMatchers.eq(CompanyRegisteredOfficeUkAddressPage), ArgumentMatchers.eq(ukAddress))(using any())
        verify(mockUserAnswers)
          .set(ArgumentMatchers.eq(CheckCompanyRegisteredOfficeAddressPage), ArgumentMatchers.eq(true))(using any())
      }
    }
    ...
    "must not save Company Details from Lookup when international address is returned" in {
      ...
      running(application) {
        ...
        verify(mockUserAnswers, never()).set(ArgumentMatchers.eq(CompanyNamePage), any)(using any())
        verify(mockUserAnswers, never()).set(ArgumentMatchers.eq(CompanyRegisteredOfficeUkAddressPage), any)(using
          any()
        )
        verify(mockUserAnswers, never()).set(ArgumentMatchers.eq(CheckCompanyRegisteredOfficeAddressPage), any)(using
          any()
        )
      }
    }
