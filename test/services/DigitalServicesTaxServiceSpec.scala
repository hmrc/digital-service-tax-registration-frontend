  ...
  "DigitalServicesTaxService" - {
    ...
    "when .getCompany is called" - {
      ...
      "must return an instance of Company when the connect returns Some" in {
        ...
        when(mockConnector.lookupCompany(any(), any()))
          .thenReturn(Future.successful(Some(CompanyRegWrapper(mockCompany, None, None))))

        serviceUnderTest.getCompany.futureValue mustBe Some(mockCompany)
      }
