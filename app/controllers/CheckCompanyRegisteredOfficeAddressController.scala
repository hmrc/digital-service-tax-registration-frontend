    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(CheckCompanyRegisteredOfficeAddressPage, value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(CheckCompanyRegisteredOfficeAddressPage, mode, updatedAnswers))
        )
  }
}
